...
// Phase 1 fix: was hardcoded to 'http://localhost:8000/api'.
// Using a relative path means:
//   - In dev: Vite's proxy forwards /api → localhost:8000/api  (no CORS)
//   - In prod: nginx/reverse-proxy handles /api → backend container
const API_BASE = '/api';
...
function App() {
    ...
    const computeRoute = useCallback(async () => {
        ...
        try {
            ...
            setSelectedRoute(
                data.routes.find(r => r.profile === profile) || data.routes[0]
            );
        } catch (err) {
            // Show the real error to help with debugging.
            // In dev, if this errors, check:
            //   1. Is the backend running? (cd backend && uvicorn main:app --reload)
            //   2. Is the DB seeded? (cd data_pipeline && python osm_loader.py)
        ...
        <div className="app">
            <div className="nav-bar" style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 1001 }}>
                ...
                <div className="nav-tabs">
                    ...
                    <button
                        className={`nav-tab ${view === 'heatmaps' ? 'active' : ''}`}
                        onClick={() => { setView('heatmaps'); setShowAQI(true); }}
                    >
                        🌫️ Heatmaps
                    </button>
...
// Mock routes used as fallback when the backend isn't reachable yet.
// This lets the UI be developed and demoed before Phase 1 data is seeded.
function getMockRoutes() {
    ...
    return [
        {
            route_id: 'bal', profile: 'balanced',
            geometry: { type: 'LineString', coordinates: base },
            segments: [],
            cost_breakdown: {
                total_cost: 12.5, travel_time_minutes: 22.3, distance_km: 8.7,
                avg_aqi: 95, max_aqi: 145, accident_hotspots_passed: 2,
                travel_time_cost: 5, aqi_exposure_cost: 4.2, accident_risk_cost: 3.3,
            },
            weights_used: { alpha: 0.4, beta: 0.3, gamma: 0.3 },
        },
        {
            route_id: 'fast', profile: 'fastest',
            geometry: { type: 'LineString', coordinates: base.map(([a, b]) => [a + 0.006, b + 0.002]) },
            segments: [],
            cost_breakdown: {
                total_cost: 8.1, travel_time_minutes: 18.5, distance_km: 7.2,
                avg_aqi: 130, max_aqi: 200, accident_hotspots_passed: 5,
                travel_time_cost: 8.1, aqi_exposure_cost: 0, accident_risk_cost: 0,
            },
            weights_used: { alpha: 1, beta: 0, gamma: 0 },
        },
        {
            route_id: 'safe', profile: 'safest',
            geometry: { type: 'LineString', coordinates: base.map(([a, b]) => [a - 0.008, b - 0.003]) },
            segments: [],
            cost_breakdown: {
                total_cost: 15.2, travel_time_minutes: 28.1, distance_km: 10.3,
                avg_aqi: 72, max_aqi: 100, accident_hotspots_passed: 0,
                travel_time_cost: 2.8, aqi_exposure_cost: 1.5, accident_risk_cost: 10.9,
            },
            weights_used: { alpha: 0.2, beta: 0.1, gamma: 0.7 },
        },
        {
            route_id: 'health', profile: 'healthiest',
            geometry: { type: 'LineString', coordinates: base.map(([a, b]) => [a - 0.012, b + 0.005]) },
            segments: [],
            cost_breakdown: {
                total_cost: 14.8, travel_time_minutes: 32, distance_km: 11.5,
                avg_aqi: 55, max_aqi: 78, accident_hotspots_passed: 1,
                travel_time_cost: 1.6, aqi_exposure_cost: 11.2, accident_risk_cost: 2,
            },
