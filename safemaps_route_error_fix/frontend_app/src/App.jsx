...
export default function App() {
    ...
    // Bug fix: the old code did `(await resp.json()).detail` unconditionally
    // on any non-OK response. FastAPI returns a JSON body with a `detail`
    // field for *handled* errors (HTTPException), but an unhandled server-side
    // exception falls through to Starlette's default 500 handler, which
    // returns a *plain-text* "Internal Server Error" body, not JSON. Calling
    // resp.json() on that threw its own "Unexpected token 'I'..." parse
    // error, which masked the real failure. This reads the body once as text
    // and only tries to parse it as JSON, so a non-JSON error body degrades
    // gracefully instead of throwing a confusing secondary error.
    const _extractErrorMessage = async (resp) => {
        const raw = await resp.text();
        try {
            const parsed = JSON.parse(raw);
            return parsed.detail || raw || `Route failed (HTTP ${resp.status})`;
        } catch {
            if (resp.status >= 500) {
                return `Server error (HTTP ${resp.status}) while computing the route. ` +
                    `Check the backend logs for the actual exception.`;
            }
            return raw || `Route failed (HTTP ${resp.status})`;
        }
    };
    ...
    const _fetchRoute = useCallback(async (org, dst, prof, wts, depTime, custom) => {
        ...
        try {
            ...
            if (custom) {
                ...
                if (!resp.ok) throw new Error(await _extractErrorMessage(resp));
